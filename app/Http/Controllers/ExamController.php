...
class ExamController extends Controller
{
       ...
       $qnaExam = Exam::where('entrance_id',$id)->with('getQnaExam')->get();
        if(count($qnaExam) >0 ){
            ...
            if($attemptCount>=$qnaExam[0]['attempt']){
                return view("student.exam-dashboard",['success'=>false,'msg'=>"Number of Attempt complete ",'exam'=>$qnaExam]);
    ...
    public function reviewQna(Request $request){
        try{
           ...
           return response()->json(['success'=>true,'msg'=>'Qna details','data'=>$attemptData]);
